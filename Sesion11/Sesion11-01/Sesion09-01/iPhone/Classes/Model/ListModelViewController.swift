...
class ListModelViewController: UIViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let controller = segue.destination as? ModelDetailViewController {
            controller.objModel = sender as? Model
        }
    }
}
...
extension ListModelViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let objModel = self.arrayModel[indexPath.row]
        self.performSegue(withIdentifier: "ModelDetailViewController", sender: objModel)
    }
}
